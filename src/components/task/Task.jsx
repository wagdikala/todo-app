...
function Task({ task, categoryColor }) {
  const [editing, setEditing] = useState(false);
  const [text, setText] = useState(task.taskTitle);
  const { deleteTask, setComplete, doneEditing, tasksLoading } =
  ...
  return tasksLoading ? (
    ...
    <div className="task">
      <Card>
        <div className="left">
          <svg
            className={`checked ${task.categoryName.toLowerCase()}`}
          ...
          <svg
            className={`circle ${task.categoryName.toLowerCase()}`}
            style={{
              fill: editing ? "rgba(0, 0, 0, 0.2)" : `${categoryColor}`,
            }}
            viewBox="0 0 512 512"
          >
